...
class ArenaManager {
    ...
    private fun loadArenas() {
        ...
        for (arenaName in arenasSection.getKeys(false)) {
            ...
            var metaRegion: los5fantasticos.torneo.util.selection.Cuboid? = null
            val metaWorldName = config.getString("$path.meta.world")
            if (metaWorldName != null) {
                val metaWorld = Bukkit.getWorld(metaWorldName)
                if (metaWorld != null) {
                    metaRegion = los5fantasticos.torneo.util.selection.Cuboid(
                        world = metaWorld,
            ...
            if (checkpointsSection != null) {
                ...
                for (index in checkpointIndices) {
                    ...
                    val checkpoint = los5fantasticos.torneo.util.selection.Cuboid(
                        world = cpWorld,
