...
class ChainService(private val minigame: MinigameCadena) {
    ...
    private fun createLinearChains(teams: List<Team>) {
        ...
        minigame.plugin.logger.info("[ChainService] Creando enlaces lineales para ${teams.size} equipos")
        ...
        for (team in teams) {
            ...
            if (players.size < 2) {
                minigame.plugin.logger.warning("[ChainService] Equipo ${team.teamId} tiene menos de 2 jugadores, saltando")
                continue
            }
            
            minigame.plugin.logger.info("[ChainService] Creando cadena lineal para equipo ${team.teamId} con ${players.size} jugadores")
            ...
            for (i in 0 until players.size - 1) {
                ...
                minigame.plugin.logger.info("[ChainService] Enlace creado: ${playerA.name} <-> ${playerB.name}")
            }
        }
        
        minigame.plugin.logger.info("[ChainService] ✓ Enlaces lineales creados. Total de jugadores enlazados: ${playerLinks.size}")
    }
