package cl.esteban.nexo.visuals
...
class VisualChain(
    ...
    private val updatePeriodTicks = plugin.config.getLong("visuales.cadena.tick-rate", 2L)
    
    fun create() {
        val materialName = plugin.config.getString("visuales.cadena.material", "CHAIN")
        val material = try {
            Material.valueOf(materialName ?: "CHAIN")
        } catch (e: IllegalArgumentException) {
            plugin.logger.warning("Material inválido '$materialName'. Usando CHAIN.")
            Material.CHAIN
        }
        ...
        plugin.logger.info("[VisualChain] Cadena creada: ${playerA.name} <-> ${playerB.name}")
    ...
    private fun startUpdating() {
        ...
        }.runTaskTimer(plugin, 0L, updatePeriodTicks)
    }
    ...
    fun destroy() {
        ...
        plugin.logger.info("[VisualChain] Cadena destruida: ${playerA.name} <-> ${playerB.name}")
    }
}
